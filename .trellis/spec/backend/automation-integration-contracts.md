...
| Body editor has no accessible name | Require the exact contenteditable placeholder contract; fail recoverably if missing or ambiguous |
| Draft action label is `暂存离开` beside red `发布` | Click only exact `暂存离开`; never select by position, color, or partial publish text |
