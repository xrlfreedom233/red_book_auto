...
1. 真实草稿流程在导航到允许的创作页后，必须等待 `input[type=file]` 实际挂载，不得在 `domcontentloaded` 后立即判定页面变更。
2. 等待时间必须受现有 `REQUEST_TIMEOUT_MS` 约束；超时后截图并以可恢复的 `page_changed` 失败结束。
3. 登录/验证/风控页仍必须停止并返回 `human_verification`，不得尝试绕过。
4. 浏览器自动化仍只能点击名称精确为“保存草稿”的按钮；不得添加发布按钮选择器、私有接口调用、Cookie 或签名提取。
...
- [ ] 现有安全测试继续证明只存在“保存草稿”点击边界，并且导航主机受限。
