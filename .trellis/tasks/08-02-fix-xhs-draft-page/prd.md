...
4. 浏览器自动化只能点击两个精确操作：内容类型“上传图文”和当前页面中代表保存草稿的白色“暂存离开”；不得点击红色“发布”、“发布笔记”或任何其他发布操作，不得调用私有接口、提取 Cookie 或签名。
...
- [ ] 单元测试证明：正文使用实测唯一的 `div.tiptap.ProseMirror[contenteditable=true][role=textbox]` 契约填写，不依赖不存在的占位属性。
- [ ] 单元测试证明：等待超时仍返回可恢复的 `page_changed`。
- [ ] 安全测试证明点击白名单只包含“上传图文”和“暂存离开”，并且导航主机受限。
